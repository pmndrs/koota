...
const smiRegex = /^DebugPrint: Smi: (0x[0-9a-fA-F]+) \(([0-9]+)\)/;
...
const parseArray = (lines) => {
	...
	if (elementsListStartIndex !== -1) {
		...
		const elementLines = remainingLines.splice(
			elementsListStartIndex,
			metadata.elements.size + 2
		);
		value = elementLines
...
const DebugPrintBlock = (lines) => {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		// Check for Smi format first
		const smiMatch = line.match(smiRegex);
		if (smiMatch) {
			firstLine = line;
			firstMatch = smiMatch;
			isSmi = true;
		...
		// Check for regular object format
	...
	if (!firstMatch) {
		throw new Error(`Invalid DebugPrint format: "${firstLine || lines[0]}"`);
	}
	...
	if (isSmi) {
		return {
			type: 'Smi',
			hex: firstMatch[1],
			decimal: parseInt(firstMatch[2], 10),
		};
	}
