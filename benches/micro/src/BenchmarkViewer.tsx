...
const useBenchmarkContext = () => {
	...
	if (!context) {
		throw new Error('useBenchmarkContext must be used within a BenchmarkViewer');
	}
	return context;
};
...
function LogBlock({ node, depth = 0 }: { node: LogNodeData; depth?: number }) {
	...
	const [isCollapsed, setIsCollapsed] = useState(isCollapsible && node.type === 'Post Bench');
	...
	return (
		<div
			className={`border-l-2 ${
				depth === 0 ? 'border-transparent' : 'border-gray-700/50 hover:border-gray-600'
			}`}
		>
			{/* Header Row */}
			<div
					...
					{node.children && node.children.length > 0 && (
						<div className="mt-1">
							{node.children.map((childNode) => (
								<LogBlock key={childNode.id} node={childNode} depth={depth + 1} />
							))}
						</div>
					)}
				</div>
...
function NodeContent({ node }: { node: LogNodeData }) {
	...
	if (typeof node.content === 'string') {
		return <pre className="text-sm text-gray-300 whitespace-pre-wrap">{node.content}</pre>;
	}
				...
				<pre className="text-xs mt-2 max-h-64 overflow-y-auto">
					...
					{filteredValue.length > itemsToShow.length && (
						<div className="text-gray-500 italic mt-1">
							...and {filteredValue.length - itemsToShow.length} more (omitted for
							performance)
						</div>
					)}
				</pre>
...
function DebugPrintContent({ content }: { content: any }) {
	...
	// Handle Smi values (primitives without memory addresses)
	if (type === 'Smi') {
		return (
			<div className="bg-gray-800 p-3 rounded-lg border border-gray-700">
				<div className="flex items-center space-x-2 mb-2">
					<span className="text-blue-400 w-4 inline-block text-center">🔢</span>
					<span className="text-sm font-semibold text-white">Small Integer (Smi)</span>
				</div>
				<div className="text-sm font-mono text-gray-300">
					<div>Decimal: {content.decimal}</div>
					<div>Hex: {content.hex}</div>
				</div>
			</div>
		);
	}
				...
				<>
					...
					<details>
						...
						<div className="space-y-1 p-4 text-sm font-mono mb-3">
							...
							{hasOtherDetails && (
								<>
									<div className="text-xs text-gray-400 mt-2">Other Details:</div>
	...
	return (
		<div
			className={`bg-gray-800 p-3 rounded-lg border ${
				isHighlighted ? 'border-blue-500 ring-2 ring-blue-500/50' : 'border-gray-700'
			...
			<div className="flex justify-between items-center mb-2">
				<div className="flex items-center space-x-2">
					{type === 'Function' ? (
						...
						<span className="text-green-400 w-4 inline-block text-center">📦</span>
					) : (
						<span className="text-gray-400 w-4 inline-block text-center">•</span>
					)}
		...
		>
			<div className="flex justify-between items-center mb-2">
				<div className="flex items-center space-x-2">
					...
					<span className="font-bold text-lg text-red-200">DEOPTIMIZATION</span>
				</div>
			...
			<pre className="text-xs text-red-200/80 mt-2 bg-black/20 p-2 rounded">{log}</pre>
		</div>
	);
}
...
function ElementsKindTag({ kind }: { kind: string | undefined }) {
	...
	return <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${colors}`}>{kind}</span>;
}
	...
	return (
		<div
			className={`group flex items-center rounded font-mono text-sm ${
				isHighlighted ? 'bg-blue-600 text-blue-100' : 'bg-gray-700 text-gray-300'
			} ${className}`}
		>
			<button
				onClick={onClick}
