	...
	const runner = {
		...
		measure(toBench) {
			const generatorFn = function* (ctx) {
				...
				let i = 0;
				// Yields the benchmark definition to Mitata.
				// [0] provides a fresh clone of the setup data for each run to prevent side effects.
				// bench() is the actual function being measured.
				yield {
					[0]() {
						return structuredClone(setupResult);
					},
					bench(setupResult) {
