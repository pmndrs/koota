...
/** Cohen's d magnitude threshold for practical significance */
export const D_THRESHOLD = 0.2;

/**
 * Classify a pair of sample arrays using Welch's t-test + Cohen's d.
 * Both conditions must be met to declare a change (p < ALPHA AND |d| >= D_THRESHOLD).
 */
export function classify(baselineSamples: number[], candidateSamples: number[]): {
	verdict: Verdict;
	p: number;
	d: number;
} {
	...
	if (p <= ALPHA && Math.abs(d) >= D_THRESHOLD) {
