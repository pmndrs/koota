...
import { formatBytes } from '../../model/world-memory';
import { useEntityCount, useWorldTraits } from '../../state/use-world-data';
import { useWorldMemory } from '../../state/use-world-memory';
...
export function WorldDetail({ world, onSelectTrait, onSelectEntity }: WorldDetailProps) {
  ...
  const memory = useWorldMemory(world);
  ...
  return (
    <WorldProvider value={world}>
      ...
      >
        ...
        <Section label="Memory" summary={`≈ ${formatBytes(memory.totalBytes)}`}>
