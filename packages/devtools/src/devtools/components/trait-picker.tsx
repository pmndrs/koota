      ...
      {pendingRelation ? (
        <>
          <Sheet.Header onBack={handleBack}>
            Select target for{' '}
            <span className={styles.headerRelation}>{getTraitName(pendingRelation.trait)}</span>
          </Sheet.Header>
          <Sheet.Search value={filter} onChange={setFilter} placeholder="Search entities..." />
          <Sheet.List isEmpty={filteredEntities.length === 0} emptyMessage="No entities match filter">
        ...
        <>
          ...
          <Sheet.List isEmpty={sortedTraits.length === 0} emptyMessage="No traits match filter">
            {sortedTraits.map((trait) => {
              ...
              return (
                ...
                >
                  ...
                  {isRelation && <span className={styles.relationArrow}>→</span>}
                </Sheet.Item>
              );
            })}
          </Sheet.List>
