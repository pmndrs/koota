...
export function EntityDetail({ entity, onSelectTrait }: EntityDetailProps) {
  ...
  useEffect(() => {
    ...
    ctx.entityDestroySubscriptions.add(handleDestroyed);

    return () => {
      unsubscribes.forEach((unsub) => unsub());
      ctx.traitRegisteredSubscriptions.delete(handleRegistered);
      ctx.entityDestroySubscriptions.delete(handleDestroyed);
    };
  }, [entity, world]);
              ...
              <div key={traitId}>
                <div className={entityDetailStyles.traitRowContainer}>
                  <Row
                    noHover
                    onClick={
                      canExpand ? () => setExpandedTraitId(isExpanded ? null : traitId) : undefined
                    }
                    title={
                      hasDebugSource(trait) ? formatDebugSourceTitle(trait.debugSource) : undefined
                    }
                  >
                    <span className={`${badgeStyles.badge} ${badgeClasses[type]}`}>{type}</span>
                    <RowName>
                      {getTraitName(trait)}
                    ...
                    {canExpand && (
                      <span className={entityDetailStyles.expandIcon}>{isExpanded ? '▼' : '▶'}</span>
                    )}
                  </Row>
                  <button
                ...
                {isExpanded && canExpand && <TraitValueEditor entity={entity} trait={trait} />}
                {isRelation && targets.length > 0 && (
                  <div className={entityDetailStyles.relationTargetsList}>
                    {targets.map((target) => {
                      ...
                      return (
                        ...
                        >
                          <span className={entityDetailStyles.relationTargetArrow}>→</span>
