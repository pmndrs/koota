...
### Terminology
 ...
 - **Definition.** The input format users write when calling `trait()`. Can be shorthand values (`{ x: 0 }`), field descriptor format (`{ x: { kind: 'number', default: 0 } }`), or a mix of both.
- **Field Descriptor.** The canonical format for a single field: `{ kind, default, ... }`. Extensible with custom properties.
- **Schema.** A collection of field descriptors. The source of truth after parsing a Definition.
- **AoS.** Array of structures. This describes stores that are a single opaque store of structures. In JS-land this is objects, arrays, functions, etc. Anything that is ref based.
- **SoA.** Struct of arrays. Stores are columnar by field. This is efficient for CPU cache lines but also for throughput of high volume data in general.
- **Tag.** A trait with no data. It serves as a flag.
### Design Principles
...
So far all storage is a struct of arrays. Some of the arrays are scalars, some refs and others buffers. We recommend all types be scalar as much as possible, but the storage system allows all approaches. Optionally, a trait can be a array of structs, or a single ref array. This is mostly for compatibility with third party libraries built with classes that cannot be decomposed into scalars.
...
// Explicit data shape when a factory is ambiguous
const Velocity = trait<THREE.Vector3>(() => new THREE.Vector3())
...
```ts
const clamped = (value: number, min: number, max: number) => field({
  kind: 'number',
  default: () => value,
  min,
  max,
...
```ts
const zod = <T extends z.ZodType>(schema: T, defaultValue?: z.infer<T>) => field({
  kind: 'ref',
