...
/**
 * Subscribers for one trait event. Subscribers in `all` fire for every entity.
...
/**
 * Invoke subscribers in `all` before subscribers for this entity. Read the entity
 * slot afterward so earlier callbacks can unsubscribe it. Plain traits call with
 * the entity alone so callbacks keep their one argument arity.
 */
...
/**
 * An earlier subscriber may have destroyed the entity and spawned a replacement on
...
function getSlot(subs: Subscriptions, entity: Entity): Slot | undefined {
  if (subs.entityCount === 0 || !isAlive(entity)) return undefined;
  const eid = getEntityId(entity);
  const page = subs.byEntity[eid >>> 10];
...
function isAlive(entity: Entity): boolean {
  const owner = universe.pageOwners[getEntityId(entity) >>> 10];
  return owner !== null && owner !== undefined && isEntityAlive(owner.entityIndex, entity);
}
