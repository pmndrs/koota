...
export const createEntityIndex = (allocator: PageAllocator, owner: KernelContext): EntityIndex => ({
  aliveCount: 0,
  dense: [],
  sparse: [],
  reserved: new Set(),
...
export function reserveEntity(index: EntityIndex): Entity {
  if (index.aliveCount < index.dense.length) {
    const entity = index.dense[index.aliveCount];
  ...
  if (index.currentPageIdx === -1 || index.pageCursors[index.currentPageIdx] >= PAGE_SIZE) {
    const pageId = leasePage(index.allocator, index.owner);
    index.ownedPages.push(pageId);
    index.pageCursors.push(0);
    index.currentPageIdx = index.ownedPages.length - 1;
  }
  const pageId = index.ownedPages[index.currentPageIdx];
  const offset = index.pageCursors[index.currentPageIdx]++;
  const entity = packEntity(
    index.allocator.generations[pageId]![offset],
    pageId * PAGE_SIZE + offset
  );
  index.reserved.add(entity);
...
export function activateReservedEntity(index: EntityIndex, entity: Entity): Entity {
  ...
  index.sparse[getEntityId(entity)] = index.aliveCount++;
  return entity;
}
...
export function cancelReservedEntity(index: EntityIndex, entity: Entity): void {
  ...
  const generation = (getEntityGeneration(entity) + 1) & GENERATION_MASK;
  index.allocator.generations[pageId]![entityId & 1023] = generation;
  index.allocator.pageAliveCounts[pageId]--;
  index.dense.push(packEntity(generation, entityId));
}
