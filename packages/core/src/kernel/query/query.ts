import { createSparseSet, addSparse, removeSparse, clearSparse } from '../entity/entity-set';
...
import { EMPTY_MASK_PAGE, createEmptyMaskGeneration } from '../entity/paged-mask';
...
import { checkQueryTracking, seedQueryTracking } from './check-query-tracking';
...
function processTrackingModifier(
  ...
  for (const trait of modifier.traits) {
    ...
    while (group.bitmasks.length <= genId) {
      group.bitmasks.push(0);
      group.trackers.push(createEmptyMaskGeneration());
    }
    group.bitmasks[genId]! |= instance.bitflag;
...
export function createQueryInstance<T extends QueryParameter[]>(
  ...
  try {
    ...
    for (let i = 0; i < query.cleanup.length; i++) query.cleanup[i]();
    throw error;
  }
  ...
  for (const pair of query.relationFilters!) {
    getTraitInstance(ctx.traitInstances, pair.relation[$internal].trait)!.relationQueries.add(query);
  }
